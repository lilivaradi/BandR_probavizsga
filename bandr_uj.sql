...
--
-- Host: localhost:3306
-- Generation Time: Feb 19, 2026 at 11:36 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateProfile` (IN `p_user_id` INT, IN `p_bio` TEXT, IN `p_instruments` TEXT, IN `p_fav_genres` TEXT, IN `p_location` VARCHAR(100), IN `p_looking_for_band` BOOLEAN, IN `p_looking_for_fans` BOOLEAN)   BEGIN
    INSERT INTO Profiles (
        ...
        p_looking_for_band, p_looking_for_fans
    );
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateProfile` (IN `p_user_id` INT, IN `p_bio` TEXT, IN `p_instruments` TEXT, IN `p_fav_genres` TEXT, IN `p_location` VARCHAR(100), IN `p_looking_for_band` BOOLEAN, IN `p_looking_for_fans` BOOLEAN)   BEGIN
    UPDATE Profiles
    SET
        bio = p_bio,
        ...
        location = p_location,
        looking_for_band = p_looking_for_band,
        looking_for_fans = p_looking_for_fans
    WHERE user_id = p_user_id;
END$$
